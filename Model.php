...
abstract class Model
{
        ...
        foreach ($this->rules() as $attribute => $rules) {
            ...
            foreach ($rules as $rule) {
                ...
                if ($ruleName === self::RULE_UNIQUE) {
                    ...
                    $sql = Application::$app->db->prepare("SELECT * FROM $tableName WHERE $uniqueAttribute = :attr");
                    $sql->bindValue(":attr", $value);
                    $sql->execute();
                    $record = $sql->fetchObject();
                    if ($record) {
    ...
    {
        $message = $this->errorMessages()[$rule] ?? 'Unknown error';
